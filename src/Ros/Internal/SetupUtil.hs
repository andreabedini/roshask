...
addRosMsgPaths targets =
    do dir <- getCurrentDirectory
       ...
       let binfo = emptyBuildInfo { hsSourceDirs = msgPaths }
           mkHookedBuildInfo maybeBuildInfo exes = (maybeBuildInfo, map (\exe -> (mkUnqualComponentName exe, binfo)) exes)
       return $ case targets of
         LibraryAndExecutables exes -> mkHookedBuildInfo (Just binfo) exes
         Executables exes -> mkHookedBuildInfo Nothing exes
